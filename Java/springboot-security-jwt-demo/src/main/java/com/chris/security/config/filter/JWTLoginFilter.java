...
public class JWTLoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        try {
            //从权限认证信息中心读取用户名和密码
            LoginUser user = new ObjectMapper().readerFor(LoginUser.class).readValue(request.getInputStream());
            //创建身份验证token
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        //构建jwt
        String token = JWTUtils.createJWTByLoginUser(new com.chris.security.model.LoginUser(user.getUsername(), user.getPassword()));
        //把JWT添加到响应头
